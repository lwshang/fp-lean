...
For example, failure due to `Option` can be seen as throwing an exception that contains no data whatsoever, so there is an instance of {anchorTerm OptionExcept}`TODO` that allows `try ... catch ...` syntax to be used with `Option`.
...
For `MonadStateOf`, there are {anchorTerm getTheType}`TODO` with type
```anchor getTheType

```
and {anchorTerm modifyTheType}`TODO` with type
